...
| Auth | ~~WebAuthn/Passkeys~~ → **Google OAuth** — single allowed email, HMAC-signed session cookie | Phase 1 → updated |
| Route guards | `hooks.server.ts` — redirects to `/setup` if not configured, `/login` if not authed | Phase 1 |
| Setup wizard | `/setup` — SETUP_TOKEN gated → Claude token paste → vault path config | Phase 1 → updated |
| Login page | `/login` — "Sign in with Google" → OAuth redirect → callback → session cookie | Phase 1 → updated |
...
- No multi-user system, no admin panel, no per-user isolation complexity
...
### 2. Single-User, Google OAuth
Login uses **Google OAuth 2.0** (authorization code flow). Only the email address in
`ALLOWED_EMAIL` can sign in — any other Google account is rejected at the callback. A session
cookie (30-day, HttpOnly, Secure, HMAC-signed) is issued after the OAuth callback verifies the
email.

Key details:
- `auth/google.ts` — generates the authorization URL (with CSRF state), exchanges the auth code
  for an ID token, and validates the email claim
- `hooks.server.ts` — enforces the session cookie on all non-public routes
- Credentials (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `ALLOWED_EMAIL`) live in `.env` and
  are injected automatically on each deploy from GitHub Secrets (see Deploy section)
- No password, no passkey hardware dependency, no recovery CLI command needed
...
| Auth | Google OAuth 2.0 (single allowed email) | Simple, no hardware dependency, ALLOWED_EMAIL gates access |
...
└── app/
    ...
    └── src/
        ...
        │   ├── auth/
        │   │   ├── google.ts      ✓  Google OAuth: generateAuthUrl, exchangeCode, isAllowedEmail
        ...
        └── routes/
            ...
            ├── login/+page.svelte ✓  "Sign in with Google" → /api/auth/google
            ├── setup/+page.svelte ✓  wizard: SETUP_TOKEN → token → vault
            ├── monitor/           ✓  +page.svelte + +page.server.ts
            ├── settings/          ✓  +page.svelte + +page.server.ts
            └── api/
                ├── auth/google/   ✓  redirect to Google OAuth consent screen
                ├── auth/callback/ ✓  exchange code, verify email, issue session cookie
...
First visit (not set up) → /setup
  ...
  Step 1: Enter setup token
          [input: SETUP_TOKEN] — required header on all /api/auth/register requests
          Prevents anyone without the VPS-generated token from completing setup.
...
Subsequent visits → /login
  [Sign in with Google] → GET /api/auth/google → Google consent screen
  → callback to /api/auth/callback → email verified against ALLOWED_EMAIL
...
# Best practice: store in GitHub Secrets; deploy.sh injects them into .env automatically.
GOOGLE_CLIENT_ID=
GOOGLE_CLIENT_SECRET=
ALLOWED_EMAIL=you@gmail.com   # only this Google account can sign in
...
`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `ALLOWED_EMAIL` can be stored as GitHub Secrets
and are written into the server's `.env` automatically by `scripts/deploy.sh` on every CI deploy.
Manual deploys must set them in `.env` directly.
...
| Flow | What to test |
|---|---|
| Setup wizard | SETUP_TOKEN → paste Claude token → vault config → redirect to `/` |
| Login | "Sign in with Google" link visible → (full OAuth flow requires mocked callback in CI) |
...
4. ✓ Auth: originally WebAuthn/Passkeys → **replaced with Google OAuth** (`auth/google.ts`); HMAC-signed session cookie; route guard (`hooks.server.ts`)
5. ✓ Setup wizard: SETUP_TOKEN gated → Claude token paste → vault path config
6. ✓ Login page: "Sign in with Google" → `/api/auth/google` → `/api/auth/callback`
